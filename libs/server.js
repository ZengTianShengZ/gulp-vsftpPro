...
module.exports = function (options) {
    ...
    var tryBackupHtml = function (con, uploader) {
        if (backupIndexHtml && remotePlatform == 'unix') {
            // 创建 文件夹backupIndexHtml 并 将 index.html 复制到该文件夹下
            var exe = 'mkdir -p ..'+remotePath+'/backupIndexHtml && cp -f ..' + remotePath + '/index.html $_'
            con.exec(exe, function (err) {
                if (err) {
                    console.log(err)
                    gutil.log(gutil.colors.red("** backup index.html error"));
                    return con.end();
                } else {
                    gutil.log(gutil.colors.green(homePath + '** backup index.html success!'));
                }
            })
        }
    };
